"""
Command-Line Interface for CXR-Grounder: Chest X-Ray Vision-Language Finding Localization Agent.
"""
import argparse
import csv
import sys
...
def main(argv=None):
    parser = argparse.ArgumentParser(prog="cxr-multimodal-report-grounder", description="CXR-Grounder: Chest X-Ray Vision-Language Finding Localization Agent")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  CXR-GROUNDER: CHEST X-RAY VISION-LANGUAGE FINDING LOCALIZATION AGENT")
        print(f"  Domain: Medical Multimodal AI | Standard: DICOM SR / CheXpert Labeling Standards")
    ...
    if args.command == "batch":
        ...
        if not output_path.name == args.output:
            print(f"Error: Invalid output path: {args.output}", file=sys.stderr)
            return 1
    ...
    if args.command == "serve":
        try:
            ...
            app = create_app()
            if app:
                print(f"Starting CXR-Grounder: Chest X-Ray Vision-Language Finding Localization Agent on http://{args.host}:{args.port}")
